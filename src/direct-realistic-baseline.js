...
const DIRECT_ROUTES = {
  dependencies: ["workspace-dependency-index"],
  symbols: ["workspace-symbol-index"],
  search: ["workspace-search-index"],
  duplicates: ["workspace-duplicate-index"],
  lint: ["workspace-lint-index"],
  metadata: ["workspace-metadata-index"],
  report: [
...
export async function runDirectRealisticBaseline({ request, state, registry = buildRealisticRegistry() }) {
  ...
  try {
    ...
    for (const id of route) {
      ...
      outputs[id] = await capability.run(Object.freeze({
        request,
        state,
        dependencies,
