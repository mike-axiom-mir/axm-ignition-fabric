...
export class CapabilityRegistry {
  ...
  register(capability) {
    ...
    if (capability.materialize && typeof capability.materialize !== "function") throw new Error(`capability ${capability.id} materialize must be a function`);
    if (capability.release && typeof capability.release !== "function") throw new Error(`capability ${capability.id} release must be a function`);
    this.capabilities.set(capability.id, Object.freeze({ deterministic: true, dependencies: [], resourceEstimateBytes: 0, ...capability, dependencies: [...(capability.dependencies || [])].sort() }));
  }
  all() { return [...this.capabilities.values()].sort((a, b) => a.id.localeCompare(b.id)); }
  get(id) { return this.capabilities.get(id); }
  matched(request, state) { return this.all().filter((capability) => capability.match(request, state)); }
}
...
function dependencyClosure(registry, initial) {
  ...
  while (queue.length) {
    ...
    for (const depId of current.dependencies) {
      ...
      if (!selected.has(dep.id)) { selected.set(dep.id, dep); queue.push(dep); queue.sort((a, b) => a.id.localeCompare(b.id)); }
    }
  }
...
function planRun(registry, request, state, mode) {
  ...
  return { matched, executable, materialized: mode === "eager" ? registry.all() : executable };
}
...
async function materializeCapability(capability, context) {
  ...
  if (!capability.materialize) return { instance: null, receipt: { capabilityId: capability.id, allocatedBytes: 0, elapsedMs: performance.now() - started } };
  const materialized = await capability.materialize(Object.freeze(context));
  if (!materialized || typeof materialized !== "object") throw new Error(`capability ${capability.id} materialize() must return an object`);
  ...
  return { instance: materialized.instance ?? null, receipt: { capabilityId: capability.id, allocatedBytes, elapsedMs: performance.now() - started } };
}

export async function executeIgnitionRun({ registry, request, state = {}, mode = "ignition", resourceProbe = null, stateFingerprint = null }) {
  ...
  const resolvedStateHash = stateFingerprint ?? hashValue(state);
  const requestHash = hashValue(request);
  const runId = `run-${hashValue({ requestHash, stateHash: resolvedStateHash, mode })}`;
  const plan = planRun(registry, request, state, mode);
  const ordered = topoSort(plan.executable);
  const started = performance.now();
  const outputs = {}, capabilityReceipts = [], materializationReceipts = [], resourceSnapshots = {};
  const runtimeById = new Map();
  let releaseError = null;

  resourceSnapshots.beforeMaterialize = await takeResourceSnapshot(resourceProbe, "beforeMaterialize", { mode, materializedCapabilityIds: plan.materialized.map((capability) => capability.id) });
  try {
    for (const capability of plan.materialized) {
      ...
      runtimeById.set(capability.id, instance); materializationReceipts.push(receipt);
    }
    resourceSnapshots.afterMaterialize = await takeResourceSnapshot(resourceProbe, "afterMaterialize", { mode, materializedCapabilityIds: plan.materialized.map((capability) => capability.id) });
    for (const capability of ordered) {
      ...
      const output = await capability.run(Object.freeze({ request, state, dependencies, runtime: runtimeById.get(capability.id) }));
      outputs[capability.id] = output;
      capabilityReceipts.push({ capabilityId: capability.id, dependencies: capability.dependencies, outputHash: hashValue(output), elapsedMs: performance.now() - capStarted, resourceEstimateBytes: capability.resourceEstimateBytes });
    }
  } finally {
    for (const capability of [...plan.materialized].reverse()) {
      try { if (capability.release) await capability.release(Object.freeze({ request, state, mode, runtime: runtimeById.get(capability.id) })); } catch (error) { releaseError ??= error; }
      runtimeById.delete(capability.id);
    }
    resourceSnapshots.afterRelease = await takeResourceSnapshot(resourceProbe, "afterRelease", { mode, releasedCapabilityIds: plan.materialized.map((capability) => capability.id) });
  }
  ...
  return { result: merged, receipt: {
    schema: "axm.ignition-run/v0.05", runId, mode, requestHash, stateHash: resolvedStateHash,
    stateFingerprintReused: stateFingerprint !== null,
    matchedCapabilityIds: plan.matched.map((capability) => capability.id), executedCapabilityIds: ordered.map((capability) => capability.id), materializedCapabilityIds: plan.materialized.map((capability) => capability.id),
    materializedCount: plan.materialized.length, executedCount: ordered.length,
    estimatedWorkingSetBytes: plan.materialized.reduce((sum, capability) => sum + capability.resourceEstimateBytes, 0), actualMaterializedBytes,
    materializationReceipts, capabilityReceipts, resourceSnapshots, resultHash: hashValue(merged), elapsedMs: performance.now() - started,
    releasedCapabilityIds: plan.materialized.map((capability) => capability.id)
  } };
}

export function compareEquivalentRuns(a, b) {
  return { equivalent: hashValue(a.result) === hashValue(b.result), leftHash: hashValue(a.result), rightHash: hashValue(b.result), estimatedWorkingSetDeltaBytes: a.receipt.estimatedWorkingSetBytes - b.receipt.estimatedWorkingSetBytes, actualMaterializedDeltaBytes: a.receipt.actualMaterializedBytes - b.receipt.actualMaterializedBytes, materializedCountDelta: a.receipt.materializedCount - b.receipt.materializedCount, executedCountDelta: a.receipt.executedCount - b.receipt.executedCount };
}
